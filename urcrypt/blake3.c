...
typedef output_t blake3_output_t;
                            ...
                            uint8_t flags)
{
  ...
  blake3i_chunk_state_init(&cs, key, flags);
  blake3i_chunk_state_update(&cs, chunk, chunk_length);
  return blake3i_chunk_state_output(&cs);
}
                             ...
                             uint8_t flags)
{
  ...
  return blake3i_parent_output(block, key, flags);
}
...
urcrypt_blake3_xof(blake3_output_t o,
                   ...
                   uint8_t *out)
{
  blake3i_output_root_bytes(&o, 0, out, out_length);
  urcrypt__reverse(out_length, out);
}
